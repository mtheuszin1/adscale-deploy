...
const PricingPage: React.FC<PricingPageProps> = ({ onSubscribe }) => {
  ...
  if (isSuccess) {
    return (
      <div className="min-h-screen bg-[#020617] flex items-center justify-center p-6 text-center animate-in fade-in duration-500">
        <div className="max-w-md space-y-8">
          ...
          <div className="space-y-4">
            <h2 className="text-4xl font-black italic uppercase tracking-tighter text-white">Parabéns!</h2>
  ...
  return (
    <div className="min-h-screen bg-[#020617] py-20 px-6 overflow-x-hidden">
      <div className="max-w-5xl mx-auto relative">
        ...
        <div className="text-center mb-16 relative z-10">
          ...
          <h1 className="text-5xl md:text-7xl font-black tracking-tighter mb-8 leading-none italic uppercase">
            Libere a <span className="text-blue-600">Inteligência</span> <br /> de Elite agora.
          </h1>
          <p className="text-slate-500 text-lg md:text-xl max-w-2xl mx-auto font-medium italic">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-10 relative z-10">
          {plans.map((plan, i) => (
            <div 
              key={i} 
              className={`relative bg-[#0B0F1A]/60 backdrop-blur-xl border rounded-[48px] p-12 flex flex-col transition-all hover:scale-[1.02] ${
                plan.recommended ? 'border-blue-500 shadow-2xl shadow-blue-600/10' : 'border-slate-800'
              }`}
            >
              {plan.recommended && (
              ...
              <div className="mb-10">
                <h3 className="text-xl font-black uppercase italic text-white mb-6 tracking-tight">{plan.name}</h3>
                <div className="flex items-end gap-2 mb-3">
                  <span className="text-6xl font-black font-mono tracking-tighter text-white">{plan.price}</span>
                  <span className="text-slate-500 mb-2 font-bold uppercase text-xs tracking-widest">{plan.period}</span>
                </div>
              ...
              <button 
                ...
                className={`w-full py-6 rounded-3xl font-black text-xs uppercase tracking-[0.2em] transition-all active:scale-95 flex items-center justify-center gap-3 italic ${
                  plan.recommended 
                    ? 'bg-emerald-600 text-white hover:bg-emerald-500 shadow-xl shadow-emerald-600/30' 
